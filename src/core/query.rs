...
pub fn execute_query(
    ...
    if fields.is_empty() && aggregations.is_empty() {
        return Ok(QueryResult { headers: vec![], rows: vec![], total_found: 0, execution_time_micros: 0, debug_info: None });
    }
    ...
    Ok(QueryResult { headers: fields.to_vec(), rows, total_found, execution_time_micros: start_time.elapsed().as_micros(), debug_info: None })
}
...
fn handle_aggregations(
    ...
    Ok(QueryResult { headers, rows, total_found: 0, execution_time_micros: 0, debug_info: None })
}
