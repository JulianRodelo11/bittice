...
fn exact_index_migration_v1_to_v2() {
    ...
    let mut idx = ExactIndex::open(&path).expect("load v1");
    assert_eq!(idx.len(), 2);
    ...
    let path_v3 = tmp.path().join("exact_v3.idx");
    idx.save(Some(&path_v3)).expect("save v3");

    let raw_bytes = fs::read(&path_v3).unwrap();
    assert_eq!(&raw_bytes[..4], b"BTXI", "v3 magic must be BTXI");
    assert_eq!(raw_bytes[4], 3, "version byte must be 3");

    let reloaded = ExactIndex::open(&path_v3).expect("reload v3");
...
fn exact_index_migration_nfc_collapse() {
    ...
    let idx = ExactIndex::open(&path).expect("load");
...
fn exact_index_build_and_query_roundtrip() {
    ...
    // Verify .idx file has BTXI v3 magic.
    ...
    assert_eq!(raw[4], 3, "exact_Email.idx version must be 3 (v3)");
