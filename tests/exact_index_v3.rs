...
fn v3_migrate_from_v2() {
    ...
    use bittice::core::storage::exact_index_io::save_exact_index;
    use bittice::core::storage::exact_index_v3::migrate::migrate_exact_index_to_v3;

    let tmp = tempfile::tempdir().unwrap();
    let path = tmp.path().join("migrate_v2.idx");

    // Build a v2 file using existing save_exact_index.
    let mut idx = ExactIndex::new();
    idx.insert("user@example.com", vec![(1u64, bm(&[1, 2, 3]))]);
    idx.insert("admin@example.com", vec![(2u64, bm(&[10, 20]))]);
    idx.insert("noreply@example.com", vec![(3u64, bm(&[100]))]);
    save_exact_index(&path, &idx).expect("save v2");
